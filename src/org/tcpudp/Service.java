...
public class Service extends Thread{
     ...
     */
    public Service(TCP tcp, RoutageTable routage_table, Identity identity_other, java.net.Socket socket, Flux flux, java.util.List<IReceived> list_i_received, java.util.List<IStatutClient> list_i_statut, java.util.List<ILog> list_i_log) {
        this.tcp                = tcp;
     ...
     */
    private String  GENERATE_CHAIN(int length) {
        String chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"; // Tu supprimes les lettres dont tu ne veux pas
        
        String pass = "";
        for (int x = 0; x < length; x++)
            pass += chars.charAt((int) Math.floor(Math.random() * chars.length()));
     ...
     */
    private void    NEW_CLIENT_HAS_CONNECTED(Identity new_client){
     ...
     */
    private void    NEW_CLIENT_HAS_DISCONNECTED(Identity new_client, EDisconnectReason reason){
    ...
    private void    LISTEN(){
        ...
        }catch(org.tcpudp.ReceivingIOException e){
        ...
        if (receivedTrame != null) {
            ...
            if (IS_INTERN_TRAME(receivedTrame)){
                ...
                if(receivedTrame instanceof Log){
                    for(int i=0;i<list_i_log.size();i++){
     ...
     * @param receivedTrame Correspond à la trame reçue
     */
    private void    REDIRECT_EXTERN_TRAME(Object receivedTrame){
        //Redirige les trame externe
        for(int i=list_i_received.size()-1; i>-1; i--) 
            ...
            synchronized(routage_table){
                for(int i=0;i<routage_table.size();i++){
                    ...
                    if(s.isLogRedirection()){
                        ...
                        }catch(org.tcpudp.SendingException e){
